...
char mat[maxn][maxn]; int n, m;

vector<Mint> dp[maxn][maxn]; // I am at (i, j), (k, )
bitset<maxn> vis[maxn][maxn] = { 0 }; // vis[i][j][k]
// i + j == (n-1 - k) + (m - 1 - l)
// l =  n - 1 - k + m - 1 - i - j
// l = n + m - (k + i + j + 2)
...
Mint rec(int i, int j, int k) {
    int l = n + m - (k + i + j + 2);
    int mink = max(n - 1 - i - j, 0);
    if(l < 0 || l >= m || k < i || l < j || mat[i][j] != mat[k][l]) return 0;
    if(k - i + l - j < 2) return 1;
    if(!vis[i][j][k]) {
        vis[i][j][k] = true;
        dp[i][j][k-mink] = rec(i+1, j, k) + rec(i, j+1, k) + rec(i+1, j, k-1) + rec(i, j+1, k-1);
    }
    return dp[i][j][k-mink];

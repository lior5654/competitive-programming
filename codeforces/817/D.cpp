...
ll a[maxn];
int lb[maxn]; int ls[maxn]; int rb[maxn]; int rs[maxn];
int n;
ll res = 0;
void solve()
{
    ...
    rep(i, n)
    {
        ...
        lb[i] = (s1.empty()) ? -1 : s1.top();
        s1.push(i);
        while(!s2.empty() && a[s2.top()] > a[i]) s2.pop();
        ls[i] = (s2.empty()) ? -1 : s2.top();
        s2.push(i);
    }
    ...
    for(int i = n - 1; i >= 0; --i)
    {
        ...
        s1.push(i);
        while(!s2.empty() && a[s2.top()] >= a[i]) s2.pop();
        rs[i] = (s2.empty()) ? n : s2.top();
        s2.push(i);
    }
    rep(i, n)
    { // [] (]
        res += ((ll)(i - lb[i]) * (rb[i] - i) - (ll)(i - ls[i]) * (rs[i] - i)) * a[i];
    }
    cout << res << '\n';

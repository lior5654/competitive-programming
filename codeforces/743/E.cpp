...
void solve()
{
    ...
    rep(i, n)
    {
        ...
        seen[a[i]] = true;
        mem[a[i]].push_back(i);
        memloc[i] = mem[a[i]].size() - 1;
    }
    /*
    for(int mask = 1; mask < 256 - 1; ++mask)
    {
        bool good = true;
        ...
        if(good)
        {
            res = max(res, popcnt(mask));
        }
    }*/
    res = seen.count();
    if(res == 8)
    {
        for(int c = 1; c <= n / 4 + 1; ++c)
        {
            //cout << res << endl;
            dp[0][0] = 0;
            for(int i = 0; i < n; ++i)
            {
                for(int mask = 0; mask < 256; ++mask)
                {
                    ...
                    if(mask&(1<<(a[i]-1)))
                    {
                        if(memloc[i]>=c-1)
                        {
                            if(!(mask^(1<<(a[i]-1))) || dp[mem[a[i]][memloc[i]-c+1]][mask^(1<<(a[i]-1))])
                            {
                                dp[i+1][mask] = max(dp[i+1][mask], dp[mem[a[i]][memloc[i]-c+1]][mask^(1<<(a[i]-1))] + c);
                            }
                            if(memloc[i]>=c)
                            {
                                if(!(mask^(1<<(a[i]-1))) || dp[mem[a[i]][memloc[i]-c]][mask^(1<<(a[i]-1))])
                                {
                                    dp[i+1][mask] = max(dp[i+1][mask], dp[mem[a[i]][memloc[i]-c]][mask^(1<<(a[i]-1))] + c + 1);
                                }
                            }
                        }
                    }

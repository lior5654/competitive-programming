...
vi g[maxn]; int n, m, q;
int next_id = n+1;
vi g2[maxc]; int tsz = n;
int d[maxc];
int p[maxh][maxc];
int cliq[maxn]; int cqz = 0;
bitset<maxn> vis = 0;
bitset<maxn> act = 0;
bitset<maxn> dvis = 0;
pi edges[2*maxc]; int ecnt = 0;
int deg[maxc];
void dfs2(int e) {
    act[e] = 0;
    cliq[cqz++] = e;
...
void dfs(int c, int p=0) {
    ...
    for(auto e : g[c]) {
        if(act[e]) {
            ...
            for(int i = 0; i < cqz; ++i) {
                int f = cliq[i];
                edges[ecnt] = {next_id, f};
                edges[ecnt+1] = {f, next_id};
                ecnt += 2;
                deg[next_id]++;
                deg[f]++;
...
void dfs3(int c, int par=0, int dd=0) {
    p[0][c] = par;
    d[c] = dd;
    for(auto e : g2[c]) {
        if(e!=par) {

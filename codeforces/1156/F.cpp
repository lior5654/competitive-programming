...
Mint dp[2][maxn][maxn]; // 1: you have a free pick
int cnt[maxn];
int pre[maxn];
...
you are at i and you are not allowed to bpick any of the j things before you
*/
...
void solve() {
    ...
    for(int i = n; i >= 1; --i) {
        rep(j, pre[i]+1) {
            Mint total = j + (n - pre[i]);
            // j/total -> losing
            // 
            if(total == 0) continue;

            dp[0][i][j] = ((total-j-cnt[i]) * inv[(int)total]) * dp[1][i+1][j+cnt[i]]; // picked something bigger
            if((int)total > 1) {
                dp[0][i][j] += (cnt[i] * inv[(int)total]) * ((cnt[i]-1)*(inv[(int)total-1]) + ((total-cnt[i]-j)*(inv[(int)total-1])) * (dp[1][i+1][j+cnt[i]-1])); // picked me
            }
            if(total != j) {
                dp[1][i][j] = ((total-j-cnt[i])*(inv[(int)total-j])) * (dp[1][i+1][j+cnt[i]]); // something bigger
                if((int)total > 1) {
                    dp[1][i][j] += ((cnt[i])*inv[(int)total-j]) * ((cnt[i]-1)*inv[(int)total-1] + (total-cnt[i]-j)*inv[(int)total-1] * (dp[1][i+1][j+cnt[i]-1]));  // me
                }
            }
            //dp[1][i][j] = dp[a[i]==a[i+1]][i+1][j] + (((Mint)1)/((Mint)n-(Mint)j));
        }
    }
    cout << dp[0][1][0] << '\n';
}
